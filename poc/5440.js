...
function Check(script) {
	if (debug) { print("Testing:", script); }
	try { eval("'use types'; " + script); } catch(e) {}
}

function CheckInvalidAmbient(decl, full=null, preamble) {
	preamble = typeof(preamble) === "undefined" ? '' : preamble + '; ';
	Check(preamble + "declare " + decl);
	if (typeof(full) === "undefined") full = decl;
	if (typeof(full) === "null") return;
	Check(preamble + "declare " + decl + "; " + full);
}

function CheckInvalidAmbientInInnerScope(decl, full=null, preamble) {
	preamble = typeof(preamble) === "undefined" ? '' : preamble + '; ';
	Check(preamble + "{ declare " + decl + "}");
	Check(preamble + "try { declare " + decl + "} catch (x) {}");
	Check(preamble + "function f() { declare " + decl + "}");
	Check(preamble + "for (declare " + decl + " of []) {} }");
}
